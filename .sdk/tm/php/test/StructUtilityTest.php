...
// Gemini SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = GeminiOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(GeminiOmni::structfix($now, Struct::undef()))
                        !== json_encode(GeminiOmni::structfix($val, Struct::undef()))) {
