...
class FtCtrl extends GeminiControl
{
...
class FtClient
{
    ...
    private GeminiUtility $utility;

    public function __construct(GeminiUtility $utility, array $options)
    {
    ...
    public function get_utility(): GeminiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GeminiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GeminiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GeminiConfig::make_feature($name);
    ...
    private static function buildUrl(GeminiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GeminiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GeminiResponse($fetched);
        }
    ...
    private function fail(GeminiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GeminiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
