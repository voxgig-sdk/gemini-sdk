...
// Gemini SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class GeminiMakePoint
{
    public static function call(GeminiContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof GeminiError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = GeminiHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = GeminiHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
