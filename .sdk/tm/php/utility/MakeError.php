...
// Gemini SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class GeminiMakeError
{
    public static function call(?GeminiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GeminiContext([], null);
        }
        $op = $ctx->op ?? new GeminiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GeminiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GeminiError) ? $err->msg : (string)$err;
        $msg = "GeminiSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new GeminiError('', $msg, $ctx);
