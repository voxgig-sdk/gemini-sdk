...
(0, node_test_1.describe)('ModelDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-model', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-model', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = "gemini-3.6-flash";
        }
        else {
            params.id = 'direct01';
        }
...
function liveScenariosActive() { return false && process.env.GEMINI_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.GeminiSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.GEMINI_APIKEY,
        }));
        let idmap = env['GEMINI_TEST_MODEL_ENTID'];
    ...
    const client = new __1.GeminiSDK({
        base: 'http://localhost:8080',
