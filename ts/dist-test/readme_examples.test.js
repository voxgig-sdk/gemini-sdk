...
const TEST_SEED = { "entity": { "embed_content": { "test01": { "id": "test01" } }, "generate_content": { "test01": { "id": "test01" } }, "interaction": { "test01": { "id": "test01" } }, "list_model": { "test01": { "id": "test01" } }, "model": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+GeminiSDK\b/.test(code) ||
        /\bGeminiSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+GeminiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/GeminiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
