...
The TypeScript SDK for the Gemini API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.EmbedContent()` — each with a small set of operations (`list`, `load`, `create`)
...
```ts
const client = new GeminiSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `GeminiSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `GeminiSDK.test(testopts?, sdkopts?)` | `GeminiSDK` | Create a test-mode client. |
...
| `client` | `client(): GeminiSDK` | Return the parent SDK client. |
...
Create an instance: `const embed_content = client.EmbedContent()`
...
Create an instance: `const generate_content = client.GenerateContent()`
...
Create an instance: `const list_model = client.ListModel()`
