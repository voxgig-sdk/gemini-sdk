...
func TestListModelEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.ListModel(nil)
		if ent == nil {
			t.Fatal("expected non-nil ListModelEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := list_modelBasicSetup(nil)
		...
		for _, _op := range []string{} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "list_model." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GEMINI_TEST_LIST_MODEL_ENTID JSON to run live")
			return
		}
		...
		_ = listModelRef01Data

	})
}

func list_modelBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "list_model", "ListModelTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read list_model test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse list_model test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GEMINI_TEST_LIST_MODEL_ENTID")
