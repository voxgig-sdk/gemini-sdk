...
func TestEmbedContentEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.EmbedContent(nil)
		if ent == nil {
			t.Fatal("expected non-nil EmbedContentEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := embed_contentBasicSetup(nil)
		...
		for _, _op := range []string{"create"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "embed_content." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set GEMINI_TEST_EMBED_CONTENT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func embed_contentBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "embed_content", "EmbedContentTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read embed_content test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse embed_content test data: " + err.Error())
	}
	...
	entidEnvRaw := os.Getenv("GEMINI_TEST_EMBED_CONTENT_ENTID")
