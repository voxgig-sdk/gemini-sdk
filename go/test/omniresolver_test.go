...
import (
	...
	omni "github.com/voxgig-sdk/gemini-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GeminiError); is {
				out := map[string]any{
					"name":    "GeminiError",
...
func omniCtx(arg any, client *sdk.GeminiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GeminiSDK); is {
			client = live
