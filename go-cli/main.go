// Command Gemini-cli is a CLI + REPL for the Gemini SDK,
...
const entitiesHelp = "embed_content generate_content interaction list_model model"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GEMINI_APIKEY (api key), GEMINI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GEMINI_APIKEY carries the API key and
	// GEMINI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GEMINI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GEMINI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGeminiSDK(opts)
