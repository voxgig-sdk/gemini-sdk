...
boru-driven command-line client **and** interactive REPL for the Gemini
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gemini-cli)
make build
...
GEMINI_BASE=https://api.example.com ./gemini-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gemini-cli
   ```
...
Run with no arguments to open a REPL (prompt `gemini>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `embed_content`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/gemini-cli`. |
...
(`./gemini-cli <expr>`) and interactively (the REPL), and expressions compose the
