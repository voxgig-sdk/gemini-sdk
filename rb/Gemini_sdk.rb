...
class GeminiSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = GeminiHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = GeminiHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, GeminiFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = GeminiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = GeminiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = GeminiHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = GeminiHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = GeminiError.new(
        "graphql_error", "GeminiSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.EmbedContent.list / client.EmbedContent.load({ "id" => ... })
  def EmbedContent(data = nil)
    require_relative 'entity/embed_content_entity'
    EmbedContentEntity.new(self, data)
  end


  # Canonical facade: client.GenerateContent.list / client.GenerateContent.load({ "id" => ... })
  def GenerateContent(data = nil)
    require_relative 'entity/generate_content_entity'
    GenerateContentEntity.new(self, data)
  end
  ...
  # Canonical facade: client.ListModel.list / client.ListModel.load({ "id" => ... })
  def ListModel(data = nil)
    require_relative 'entity/list_model_entity'
    ListModelEntity.new(self, data)
  end
