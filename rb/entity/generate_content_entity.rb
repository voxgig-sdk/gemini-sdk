...
class GenerateContentEntity
  ...
  def make
    opts = @_entopts.dup
    GenerateContentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = GeminiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GenerateContent, Hash] the current GenerateContent data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = GeminiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GenerateContent fields)
  def match_get
  ...
  #
  # @param reqdata [GenerateContentCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [GenerateContent, Hash] the created GenerateContent; raises GeminiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = GeminiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
