...
type Args struct {
	Entity string         `json:"entity" jsonschema:"embed_content | generate_content | interaction | list_model | model"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.GeminiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "gemini_list",
		Description: "List records from Gemini. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "gemini_load",
		Description: "Load a single record from Gemini. " +
...
func runOp(client *sdk.GeminiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.GeminiSDK, name string) (sdk.GeminiEntity, error) {
	switch strings.ToLower(name) {
	case "embed_content":
		return client.EmbedContent(nil), nil
	case "generate_content":
		return client.GenerateContent(nil), nil
	case "interaction":
		return client.Interaction(nil), nil
	case "list_model":
		return client.ListModel(nil), nil
	case "model":
