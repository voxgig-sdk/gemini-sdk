...
[MCP](https://modelcontextprotocol.io) server exposing the Gemini SDK as
two agent tools — `gemini_list` and `gemini_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gemini-mcp)
make build

# 2. Provide credentials via the environment
export GEMINI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user gemini \
  -- /absolute/path/to/gemini-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./gemini-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// gemini_list: first page of records
{ "entity": "model" }
{ "entity": "model", "query": { } }

// gemini_load: one record by id
{ "entity": "model", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gemini-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user gemini \
     -- "$PWD"/dist/*/gemini-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `gemini_list` and `gemini_load` tools now appear
   in new sessions. Ask the agent to *"list model using gemini"*
   and it calls `gemini_list` with `{"entity":"model"}`.
...
```sh
export GEMINI_APIKEY=sk_live_xxx            # API key
export GEMINI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `gemini_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `gemini_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GEMINI_APIKEY` | API key sent with every request. |
| `GEMINI_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

embed_content | generate_content | interaction | list_model | model

### Smoke test via HTTP (raw JSON-RPC)

```sh
./gemini-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gemini_load","arguments":{"entity":"model","query":{"id":1}}}}'
```
