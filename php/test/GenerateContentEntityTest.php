...
class GenerateContentEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GeminiSDK::test(null, null);
        $ent = $testsdk->GenerateContent(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = generate_content_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "generate_content." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GEMINI_TEST_GENERATE_CONTENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $generate_content_ref01_ent = $client->GenerateContent(null);
...
function generate_content_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/generate_content/GenerateContentTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["generate_content01", "generate_content02", "generate_content03", "model01", "model02", "model03"] as $k) {
    ...
    $entid_env_raw = getenv("GEMINI_TEST_GENERATE_CONTENT_ENTID");
