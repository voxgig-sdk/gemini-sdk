...
class EmbedContentEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = GeminiSDK::test(null, null);
        $ent = $testsdk->EmbedContent(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = embed_content_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "embed_content." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set GEMINI_TEST_EMBED_CONTENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $embed_content_ref01_ent = $client->EmbedContent(null);
...
function embed_content_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/embed_content/EmbedContentTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["embed_content01", "embed_content02", "embed_content03", "model01", "model02", "model03"] as $k) {
    ...
    $entid_env_raw = getenv("GEMINI_TEST_EMBED_CONTENT_ENTID");
