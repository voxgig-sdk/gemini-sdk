...
{
    private static function utility(): GeminiUtility
    {
        return new GeminiUtility();
    }
    ...
    private static function ctx(array $over = []): GeminiContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GeminiContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GeminiOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GeminiError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GeminiOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GeminiMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GeminiOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GeminiMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GeminiOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GeminiMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeminiSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GeminiMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = GeminiMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GeminiSpec(['step' => 's']),
            'response' => new GeminiResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GeminiResult([]),
        ]);
        [, $err] = GeminiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GeminiSpec(['step' => 's']),
            'response' => new GeminiResponse(self::resp(200, ['v' => 1])),
            'result' => new GeminiResult([]),
        ]);
        [, $err] = GeminiMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GeminiSpec(['step' => 's']),
            'response' => new GeminiResponse(self::resp(200, ['v' => 2])),
            'result' => new GeminiResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GeminiMakeResponse::call($ctx);
    ...
    {
        $preset = new GeminiResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GeminiSpec([]),
            'response' => new GeminiResponse([]),
            'result' => new GeminiResult([]),
        ...
        [$got, $err] = GeminiMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GeminiResult([])]);
        [, $err] = GeminiMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GeminiSpec([]), 'result' => null]);
        [, $err] = GeminiMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeminiSpec(['step' => 's']),
            'result' => new GeminiResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GeminiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeminiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GeminiSpec(['step' => 's']),
            'result' => new GeminiResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GeminiOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GeminiMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GeminiResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GeminiSpec([]),
            'result' => new GeminiResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GeminiMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeminiMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GeminiError('boom', 'boom');
        $utility->fetcher = function (GeminiContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeminiSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GeminiMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GeminiContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GeminiContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GeminiSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GeminiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GeminiResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GeminiSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GeminiMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GeminiMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GeminiMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GeminiResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GeminiDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GeminiResult(['ok' => false])]);
        $this->expectException(GeminiError::class);
        GeminiDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GeminiResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GeminiMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GeminiBaseFeature();
        $b = new GeminiBaseFeature();
        GeminiFeatureAdd::call($ctx, $a);
        GeminiFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GeminiBaseFeature
    {
        $f = new GeminiBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GeminiFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GeminiContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GeminiSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GeminiPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GeminiResponse(['headers' => null]),
            'result' => new GeminiResult([]),
        ]);
        GeminiResultHeaders::call($ctx);
