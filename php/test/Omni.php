...
//    only - a typed GeminiContext there would make every ctx
...
final class GeminiOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new GeminiContext($clean, null);
    ...
    public static function err_from_map($m): ?GeminiError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new GeminiError(is_string($code) ? $code : '', $msg);
    }
