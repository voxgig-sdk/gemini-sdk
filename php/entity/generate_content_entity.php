...
// Gemini SDK GenerateContent entity
...
class GenerateContentEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GenerateContentEntity($this->_client, $opts);
    }

    /**
     * @param GenerateContent|array $args GenerateContent data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = GeminiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GenerateContent|array The current GenerateContent data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GenerateContent fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = GeminiHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GenerateContent fields).
     */
    ...
    /**
     * Create a new GenerateContent.
     *
     * @param GenerateContentCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed GenerateContentCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GenerateContent|array The created GenerateContent as an assoc-array at the
     *   SDK boundary; throws GeminiError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = GeminiHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
