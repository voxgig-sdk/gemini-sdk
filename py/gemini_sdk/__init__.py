...
from gemini_sdk.core.spec import GeminiSpec
from gemini_sdk.core import helpers

# Load utility registration (populates Utility._registrar)
from gemini_sdk.utility import register

# Load features
from gemini_sdk.feature.base_feature import GeminiBaseFeature
from gemini_sdk.features import _has_feature, _make_feature
...
class GeminiSDK:
    ...
    def get_utility(self):
        return GeminiUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("GeminiSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def EmbedContent(self, data=None) -> "EmbedContentEntity":
        """Entity factory: client.EmbedContent().list() / client.EmbedContent().load({"id": ...})."""
        from gemini_sdk.entity.embed_content_entity import EmbedContentEntity
        return EmbedContentEntity(self, data)


    def GenerateContent(self, data=None) -> "GenerateContentEntity":
        """Entity factory: client.GenerateContent().list() / client.GenerateContent().load({"id": ...})."""
        from gemini_sdk.entity.generate_content_entity import GenerateContentEntity
        return GenerateContentEntity(self, data)


    def Interaction(self, data=None) -> "InteractionEntity":
        """Entity factory: client.Interaction().list() / client.Interaction().load({"id": ...})."""
        from gemini_sdk.entity.interaction_entity import InteractionEntity
        return InteractionEntity(self, data)


    def ListModel(self, data=None) -> "ListModelEntity":
        """Entity factory: client.ListModel().list() / client.ListModel().load({"id": ...})."""
        from gemini_sdk.entity.list_model_entity import ListModelEntity
        return ListModelEntity(self, data)


    def Model(self, data=None) -> "ModelEntity":
        """Entity factory: client.Model().list() / client.Model().load({"id": ...})."""
        from gemini_sdk.entity.model_entity import ModelEntity
        return ModelEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "GeminiSDK":
        if sdkopts is None:
            sdkopts = {}
