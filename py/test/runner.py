...
class GeminiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GeminiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GeminiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GeminiTestRunner.getenv("GEMINI_TEST_LIVE")
        override = GeminiTestRunner.getenv("GEMINI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GeminiTestRunner.getenv(key)
        ...
        explain = GeminiTestRunner.getenv("GEMINI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GEMINI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GeminiTestRunner._test_control is not None:
            return GeminiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GeminiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GeminiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GeminiTestRunner.load_test_control()
...
def load_env_local():
    GeminiTestRunner.load_env_local()


def env_override(m):
    return GeminiTestRunner.env_override(m)


def entity_data(v):
    return GeminiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GeminiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GeminiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GeminiTestRunner.load_test_control()


def live_client_options():
    return GeminiTestRunner.live_client_options()


def live_delay_ms():
    return GeminiTestRunner.live_delay_ms()
