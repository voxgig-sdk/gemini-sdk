...
class TestListModelEntity:

    def test_should_create_instance(self):
        testsdk = GeminiSDK.test(None, None)
        ent = testsdk.ListModel(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _list_model_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in []:
            _skip, _reason = runner.is_control_skipped("entityOp", "list_model." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GEMINI_TEST_LIST_MODEL_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        list_model_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.list_model")))
        list_model_ref01_data = None
        if len(list_model_ref01_data_raw) > 0:
            list_model_ref01_data = helpers.to_map(list_model_ref01_data_raw[0][1])



def _list_model_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/list_model/ListModelTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("GEMINI_TEST_LIST_MODEL_ENTID"))
