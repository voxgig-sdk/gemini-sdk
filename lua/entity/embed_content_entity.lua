...
local EmbedContentEntity = {}
EmbedContentEntity.__index = EmbedContentEntity


function EmbedContentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EmbedContentEntity)
  self._name = "embed_content"
  self._client = client
...
function EmbedContentEntity:get_name()
  return self._name
end


function EmbedContentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EmbedContentEntity.new(self._client, opts)
end
...
function EmbedContentEntity:mark_deleted()
  self._deleted = true
end


function EmbedContentEntity:deleted()
  return true == self._deleted
end


function EmbedContentEntity:data_set(args)
  if args ~= nil then
...
function EmbedContentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EmbedContentEntity:match_set(args)
  if args ~= nil then
...
function EmbedContentEntity:match_get()
...
function EmbedContentEntity:stream(action, args, callopts)
...
---@param reqdata EmbedContentCreateData
---@param ctrl? table
---@return EmbedContent
---@return string? err
function EmbedContentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function EmbedContentEntity:_run_op(ctx, post_done)
  local utility = self._utility
