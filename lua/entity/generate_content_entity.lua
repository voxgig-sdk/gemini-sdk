...
local GenerateContentEntity = {}
GenerateContentEntity.__index = GenerateContentEntity


function GenerateContentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GenerateContentEntity)
  self._name = "generate_content"
  self._client = client
...
function GenerateContentEntity:get_name()
  return self._name
end


function GenerateContentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GenerateContentEntity.new(self._client, opts)
end
...
function GenerateContentEntity:mark_deleted()
  self._deleted = true
end


function GenerateContentEntity:deleted()
  return true == self._deleted
end


function GenerateContentEntity:data_set(args)
  if args ~= nil then
...
function GenerateContentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GenerateContentEntity:match_set(args)
  if args ~= nil then
...
function GenerateContentEntity:match_get()
...
function GenerateContentEntity:stream(action, args, callopts)
...
---@param reqdata GenerateContentCreateData
---@param ctrl? table
---@return GenerateContent
---@return string? err
function GenerateContentEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function GenerateContentEntity:_run_op(ctx, post_done)
  local utility = self._utility
