-- Gemini SDK ListModel entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ListModelEntity = {}
ListModelEntity.__index = ListModelEntity


function ListModelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ListModelEntity)
  self._name = "list_model"
  self._client = client
...
function ListModelEntity:get_name()
  return self._name
end


function ListModelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ListModelEntity.new(self._client, opts)
end
...
function ListModelEntity:mark_deleted()
  self._deleted = true
end


function ListModelEntity:deleted()
  return true == self._deleted
end


function ListModelEntity:data_set(args)
  if args ~= nil then
...
function ListModelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ListModelEntity:match_set(args)
  if args ~= nil then
...
function ListModelEntity:match_get()
...
function ListModelEntity:stream(action, args, callopts)
...
function ListModelEntity:_run_op(ctx, post_done)
  local utility = self._utility
