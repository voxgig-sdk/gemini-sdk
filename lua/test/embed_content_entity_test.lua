...
describe("EmbedContentEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:EmbedContent(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = embed_content_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "embed_content." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GEMINI_TEST_EMBED_CONTENT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- CREATE
    local embed_content_ref01_ent = client:EmbedContent(nil)
    local embed_content_ref01_data = helpers.to_map(vs.getprop(
...
function embed_content_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/embed_content/EmbedContentTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read embed_content test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("GEMINI_TEST_EMBED_CONTENT_ENTID")
