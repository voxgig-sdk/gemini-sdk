...
The Lua SDK for the Gemini API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:EmbedContent()` — each with the same small set of operations (`list`, `load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/gemini-sdk/releases)),
...
Create an instance: `local embed_content = client:EmbedContent(nil)`
...
Create an instance: `local generate_content = client:GenerateContent(nil)`
...
Create an instance: `local list_model = client:ListModel(nil)`
...
```
lua/
├── gemini_sdk.lua    -- Main SDK module
...
The main module (`gemini_sdk`) exports the SDK constructor
